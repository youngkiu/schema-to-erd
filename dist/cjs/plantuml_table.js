...
function generatePlantUml(tableColumns) {
    const entities = Object.entries(tableColumns).reduce((acc, [tableName, { columnNames, primaryKeys }]) => (acc + generateEntity(tableName, columnNames, primaryKeys)), '');
    const allPKs = Object.entries(tableColumns).reduce((accTable, [tableName, { primaryKeys }]) => primaryKeys.reduce((accPk, primaryKey) => {
        ...
        return Object.assign(Object.assign({}, accPk), { [primaryKeyName]: foreignKey });
    }, accTable), {});
    const relations = Object.entries(tableColumns).reduce((acc, [tableName, { columnNames, primaryKeys }]) => [
...
exports.default = generatePlantUml;
//# sourceMappingURL=plantuml_table.js.map